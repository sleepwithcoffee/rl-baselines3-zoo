...
class Env2048(gym.Env):
    ...
    def __init__(self, high = 4, wide = 4, seed = None, invalid_move_warmup=16,
        ...
        # Old: When the maximum value of a block is the 15th power of 2 (32768), add 1000 of the merge reward, and end the episode.
        # New: increase the winning condition to the 20th power of 2 (1048576)
        self.__deep_ob = 16
        ...
        for temp in range(1, len(row)):
            if (row[temp]):
                if (row[temp - 1] == row[temp]):
                    ...
                    self.reward += np.log2(row[temp - 1])
                    # https://medium.com/@kotamori/total-score-formula-of-the-2048-game-d9a8c9a1f1ac
                    # The scoring rule of the game is simple: When we make a number X, we earn score X.
                    self.game_score += row[temp - 1]
                    row[temp] = 0
                    if (self.max_block < row[temp - 1]):
                        self.max_block = row[temp - 1]

                        # update penalty when reaching a new level
                        self.__penalty = -np.log2(self.max_block)*2 + self.__penalty_min
                    # Add log2(max_block) to the score when the max_block is reached
                    # This will encourage reaching higher tiles
                    self.reward += np.log2(self.max_block)
    ...
    def __check_state(self):
        ...
        if(
            ...
            self.__invalid_step > self.__invalid_move_threshold * self.num_step
        ):
            return True, self.reward + self.__penalty
        # Old: When the maximum value of a block is greater than 2048, add 10 to each merge reward, which is 10 + log2(n).
        # elif (self.__max_block >= 2048):
        #     '''check whether the max value is get the 2048 or not'''
        #     return False, self.__score + 10
        elif (self.max_block == 2 ** (self.__deep_ob - 1)):
            # End game when reached 2^20
            return True, self.reward + 1000
        elif (self.__no_rest):
    ...
    def step(self, action):
        ...
        info["total_score"] = self.total_reward
        info["max_block"] = self.max_block
        info["game_score"] = self.game_score
    ...
    def reset(self, seed=None):
        ...
        self.__penalty = self.__penalty_min
